...
#
# Limit intra/inter op threads (tune values depending on CPU cores)
import tensorflow as tf
import os
tf.config.threading.set_intra_op_parallelism_threads(1)
tf.config.threading.set_inter_op_parallelism_threads(1)

# Optional: restrict OMP threads too
...
from tensorflow.keras.layers import  Flatten, Reshape, AveragePooling1D, Input, Dense, Conv1D, MaxPooling1D, UpSampling1D
from tensorflow.keras.models import Model

# functions:
# 1. timeseries windowing and normalization
# 2. autoencoder
# 3. Mean square error  with Gaussian weights
...
class AutoEncoder:
    ...
    def get_glness(self, sdata, rate):
       if(self.ae_net == None):
